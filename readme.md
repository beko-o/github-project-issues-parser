...
Скрипт `script.py` позволяет автоматически генерировать отчёты по задачам (issues) из GitHub Projects, сохраняя данные в Google Sheets. Отчёты создаются для одного или всех assignee внутри заданного временного интервала.

## ✨ Возможности

* Постраничная выгрузка проектов и задач через GitHub GraphQL API
* Фильтрация по дате создания задач и по assignee
* Генерация отдельного Google Spreadsheet для каждого assignee (или одного для указанного пользователя)
* Автоматическое создание структуры папок в Google Drive
...
* Обработка ошибок и простой retry на критичных операциях

## 📋 Требования

* Python 3.9+
* GitHub Personal Access Token с правом `read:org` и `repo`
* Google Service Account с доступом к Google Sheets и Google Drive API
* Файл `credentials.json` сервисного аккаунта Google
...
1. Клонируйте репозиторий:
   ...
   ```bash
   git clone https://github.com/<ваш-репозиторий>/github-projects-export.git
   cd github-projects-export
   ```
